...
enum JsonType { None = 0, Number = 1, String = 2, Array = 4, Bool = 8, Null = 16, Object = 32 };

enum JsonStatus { SUCCESS = 0, ERROR_FILE_READ, ERROR_FORMAT, ERROR_NULLPTR };
...
public:
    ...
    JsonStr(char* start, size_t length) : _ptr(start), _length(length) {}
    virtual ~JsonStr()
    {
        _ptr = nullptr;
        _length = 0;
...
public:
    ...
    void setKey(char* begin, size_t length)
    {
        _key.setValue(begin, length);
    }
    void setValue(char* begin, size_t length)
    {
        _value.setValue(begin, length);
    }

...
M4AStatus ItemReferenceBox::SingleItemTypeReferenceBox::ProcessData(std::fstream& file) {
    FullBox::ProcessFullBox(file);
    auto* t = new uint8_t[sizeof(uint16_t)];
    file.read((char*)t, sizeof(uint16_t));
    fromItemID = GetValue<uint16_t>(t);
    file.read((char*)t, sizeof(uint16_t));
    referenceCount = GetValue<uint16_t>(t);
    delete[] t;
    t = new uint8_t[sizeof(uint16_t) * referenceCount];
    file.read((char*)t, sizeof(uint16_t) * referenceCount);
    toItemIDs = std::make_unique<uint16_t[]>(referenceCount);
    for (uint16_t i = 0; i < referenceCount; i++) {
        toItemIDs[i] = GetValue<uint16_t>(t + i * sizeof(uint16_t));
    }
    delete[] t;
    return SUCCESS;
}

M4AStatus ItemReferenceBox::SingleItemTypeReferenceBoxLarge::ProcessData(std::fstream& file) {
    FullBox::ProcessFullBox(file);
    auto* t = new uint8_t[sizeof(uint32_t)];
    file.read((char*)t, sizeof(uint32_t));
    fromItemID = GetValue<uint32_t>(t);
    file.read((char*)t, sizeof(uint16_t));
    referenceCount = GetValue<uint16_t>(t);
    delete[] t;
    t = new uint8_t[sizeof(uint32_t) * referenceCount];
    file.read((char*)t, sizeof(uint32_t) * referenceCount);
    toItemIDs = std::make_unique<uint32_t[]>(referenceCount);
    for (uint16_t i = 0; i < referenceCount; i++) {
        toItemIDs[i] = GetValue<uint32_t>(t + i * sizeof(uint32_t));
    }
    delete[] t;
    return SUCCESS;
}
...
M4AStatus ItemReferenceBox::ProcessData(std::fstream& file) {
    FullBox::ProcessFullBox(file);
    ProcessChildBox(file, _header.BodySize() - 4);
    return SUCCESS;
}
} // namespace myUtil

...
public:
	...
	uint32_t width() const { return _data.width; }
	uint32_t height() const { return _data.height; }
};
...
public:
	ImageStatus decode(fstream& file, uint32_t length) override;
	uint32_t maxCLL() { return MaxCLL * 0.0001; }
	uint32_t maxFALL() { return MaxFALL * 0.0001; }
};
